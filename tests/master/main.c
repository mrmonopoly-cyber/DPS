...
#define ADD_NEW_VAR(BOARD_ID, VAR, VAR_ID, SIGNED, FLOAT)                      \
  {                                                                            \
    ...
    mex.GenericPayload.dps_payload.mext_type.type = VAR_NAME,                  \
    mex.GenericPayload.dps_payload.data = new_var_name.raw_data;               \
    if (dps_master_check_mex_recv(&mex)) {                                     \
    ...
    mex.GenericPayload.dps_payload.mext_type.type = VAR_METADATA;              \
    mex.GenericPayload.dps_payload.data = new_var_metadata.raw_data;           \
    if (dps_master_check_mex_recv(&mex)) {                                     \
...
#define ADD_NEW_COM(BOARD_ID, COM, COM_ID, SIGNED, FLOAT, SIZE)                \
  {                                                                            \
    ...
    mex.GenericPayload.dps_payload.mext_type.type = COM_NAME,                  \
    mex.GenericPayload.dps_payload.data = new_com_name.raw_data;               \
    if (dps_master_check_mex_recv(&mex)) {                                     \
    ...
    mex.GenericPayload.dps_payload.mext_type.type = COM_METADATA;              \
    mex.GenericPayload.dps_payload.data = new_com_metadata.raw_data;           \
    if (dps_master_check_mex_recv(&mex)) {                                     \
...
#define CHECK_COM(INDEX, NAME, COM_ID, SIGNED, FLOAT, SIZE)                    \
  {                                                                            \
    ...
    if (strcmp(com->name, NAME) || com->metadata.full_data.com_id != COM_ID || \
...
#define SEND_COM_F(ID, VAR)                                                    \
  ...
  uint16_t com_0_f = 15;
  SEND_COM_F(18, com_0_f);
  uint8_t com_0_c = 3;
  SEND_COM_C(18, com_0_c);

  uint32_t com_1_f = 50;
  SEND_COM_F(19, com_1_f);
  uint16_t com_1_c = 40;
  SEND_COM_C(19, com_1_c);

  double com_2_f = -50;
  SEND_COM_F(23, com_2_f);
  float com_2_c = -3;
  SEND_COM_C(23, com_2_c);
